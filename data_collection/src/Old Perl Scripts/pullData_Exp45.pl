...
my @dataFileList = glob($dataDir . '/*');
my @angleList = ( 135, 112.5, 90, 67.5, 45, 22.5, -22.5, -45, -65.5, -90, -112.5, -135 );
...
open(OUT, ">Data.csv");
print OUT "Participant,Landmark,Interface,GreenX,GreenZ,YellowX,YellowY,RedX,RedZ,MarkerAngle,UserX,UserZ,ErrorX,ErrorY,ErrorMag,MarkerTime,TrialTime,AngleError\n";

foreach my $file (@dataFileList){
	if($file =~ m/.+?(\d+)\_(\d+)([NLC])\_(\d+)/){
		writeData($file, $3, $2, $4,);
	} else{
		die "$file is not in the correct format!";
	}
}
...
sub writeData {
	my ($dir, $hasLandmark, $interface, $participant) = @_;
	# print "$dir\n";
	...
	if($startMarkers[$i] =~ m/Starting.+:\((-?\d+\.?\d*), .+?, (-?\d+\.?\d*)\)\nFirst Marker:\((-?\d+\.?\d*), .+?, (-?\d+\.?\d*)\)\nSecond.+?\((-?\d+\.?\d*), .+?, (-?\d+\.?\d*)\)/)
	{
			...
			if($startTimes[$i] =~ m/Start marker time: (-?\d+\.?\d*)\nSecond marker time: (-?\d+\.?\d*)\n-+New Trial-+/){
				...
				if($responseList[$i] =~ m/Response.+\((-?\d+\.?\d*), .+?, (-?\d+\.?\d*)\) Time: (\d+\.?\d*)\n-+New Trial-+/) {
					...
					print OUT "$participant,$hasLandmark,$interface,$startX,$startY,$firstX,$firstY,$secondX,$secondZ,$markerAngle,$1,$2,$errorX,$errorY,$errorMag," . ($3 - $markerHit) . "," .($3 - $trialStart) . ",$angle\n";
				}
			}
			else {
				my ($trialStart, $markerHit) = ($1, $2);
				if($responseList[$i] =~ m/Response.+\((-?\d+\.?\d*), .+?, (-?\d+\.?\d*)\) Time: (\d+\.?\d*)\n-+New Trial-+/) {
					...
					print OUT "$participant,$hasLandmark,$interface,$startX,$startY,$firstX,$firstY,$secondX,$secondZ,$markerAngle,$1,$2,$errorX,$errorY,$errorMag, , ,$angle\n";
				}
			}
		}
	}
