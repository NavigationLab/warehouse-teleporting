...
my $dataDir = 'C://Users/Test/Desktop/teleport_pilots';
...
foreach my $ui_folder (@uiFolderList){
	...
	my $tmp1 = $tmp[-1];
	if ($tmp1 eq "Discordant" || $tmp1 eq "Hybrid" || $tmp1 eq "Concordant") {
	    my @condFolderList = glob($ui_folder . '/*');
		foreach my $cond_folder (@condFolderList) {
		    @tmp = split(/\//, $cond_folder);
			my $tmp2 = $tmp[-1];
		    if ($tmp2 eq "Training" || $tmp2 eq "Classroom" || $tmp2 eq "Field_NoLandmarks_NoFence") {
